...
async function _loadFriends() {
  ...
  try {
    ...
    // Pull live score/streak from each friend's main user document so the
    // leaderboard can show real friend points instead of 0.
    const enriched = await Promise.all(friends.map(async f => {
      try {
        if (!f.uid) return f;
        const profileSnap = await _fdb().collection('users').doc(f.uid).get();
        if (!profileSnap.exists) return f;
        const profile = profileSnap.data() || {};
        const scoreObj = profile.tf_score_v4 || {};
        const streakObj = profile.tf_streak_v6 || {};
        return {
          ...f,
          displayName: f.displayName || profile.displayName || 'Friend',
          photoURL: f.photoURL || profile.photoURL || '',
          friendTag: f.friendTag || profile.friendTag || '',
          score: Number(scoreObj.total || profile.score || f.score || 0),
          streak: Number(streakObj.count || profile.streak || f.streak || 0)
        };
      } catch (e) {
        console.warn('Could not load friend leaderboard data:', e);
        return f;
      }
    }));
    return enriched;
...
function _getChatUnread() {
  try { return gs('CHAT_UNREAD') || {}; }
  catch(e) {
    try { return JSON.parse(localStorage.getItem('CHAT_UNREAD') || '{}') || {}; }
    catch(_) { return {}; }
  }
}
function _setChatUnread(friendUid, unread=true) {
  if (!friendUid) return;
  const data = _getChatUnread();
  if (unread) data[friendUid] = true;
  else delete data[friendUid];
  try { ss('CHAT_UNREAD', data); }
  catch(e) { localStorage.setItem('CHAT_UNREAD', JSON.stringify(data)); }
  updateMessageNotifUI();
}
function _hasChatUnread(friendUid) {
  return !!_getChatUnread()[friendUid];
}
function updateMessageNotifUI() {
  const unread = _getChatUnread();
  const hasAny = Object.keys(unread).length > 0;
  const navBadge = document.getElementById('nb-msgs');
  if (navBadge) {
    navBadge.textContent = '';
    navBadge.classList.add('unread-dot-badge');
    navBadge.style.display = hasAny ? 'flex' : 'none';
  }
  document.querySelectorAll('[data-chat-uid]').forEach(card => {
    const uid = card.getAttribute('data-chat-uid');
    const dot = card.querySelector('.chat-unread-dot');
    if (dot) dot.style.display = unread[uid] ? 'inline-block' : 'none';
  });
}
function _addMessageReceivedNotif(friendUid) {
  // Keep the bell notification simple, exactly as requested.
  if (typeof addNotif === 'function') addNotif('💬', 'Message Received');
  const list = document.getElementById('notif-list');
  const dd = document.getElementById('notif-dd');
  if (list && dd && dd.classList.contains('open') && typeof renderNotifList === 'function') renderNotifList();
}
function _cleanupMessageNotificationListeners() {
  _msgNotifUnsubs.forEach(fn => { try { fn(); } catch(e) {} });
  _msgNotifUnsubs = [];
  _msgNotifReady = {};
  _msgLastSeenTime = {};
}
async function initMessageNotificationListeners() {
  if (!_uid() || !_fdb()) return;
  const friends = _friendsCache.length ? _friendsCache : await _loadFriends().catch(() => []);
  if (!friends.length) { updateMessageNotifUI(); return; }

  const existingCount = _msgNotifUnsubs.length;
  if (existingCount) _cleanupMessageNotificationListeners();

  friends.forEach(f => {
    if (!f || !f.uid) return;
    const cid = _chatId(_uid(), f.uid);
    const unsub = _fdb()
      ...
      .onSnapshot(snap => {
        ...
        if (!doc) { _msgNotifReady[f.uid] = true; return; }
        const msg = doc.data() || {};
        const t = Number(msg.time || 0);

        // First snapshot is only used as the baseline, so old messages do not spam notifications.
        if (!_msgNotifReady[f.uid]) {
          _msgNotifReady[f.uid] = true;
          _msgLastSeenTime[f.uid] = t;
          return;
        }

        if (t > (_msgLastSeenTime[f.uid] || 0)) {
          _msgLastSeenTime[f.uid] = t;
          if (msg.from && msg.from !== _uid() && activeChatId !== f.uid) {
            _setChatUnread(f.uid, true);
            _addMessageReceivedNotif(f.uid);
            if (typeof beep === 'function') beep('chat');
            if (typeof notifyIncomingMessage === 'function') notifyIncomingMessage(msg, f);
          }
        }
      }, err => console.warn('Message notification listener error', err));
    _msgNotifUnsubs.push(unsub);
  });
  updateMessageNotifUI();
}
...
async function renderMsgList() {
  ...
  friends.forEach(f => {
    const card = document.createElement('div');
    card.className = 'friend-card';
    card.setAttribute('data-chat-uid', f.uid);
    const av = f.photoURL
      ? `<img src="${f.photoURL}" style="width:38px;height:38px;border-radius:50%;object-fit:cover" referrerpolicy="no-referrer"/>`
      : `<div style="width:38px;height:38px;border-radius:50%;background:var(--accent);display:flex;align-items:center;justify-content:center;font-weight:700;color:#fff">${(f.displayName||'?')[0].toUpperCase()}</div>`;
    card.innerHTML = `<div class="friend-av">${av}</div><div class="friend-info"><div class="friend-name">${escHtml(f.displayName)}</div><div class="friend-meta">Tap to chat</div></div><div class="friend-actions"><span class="chat-unread-dot" style="display:${_hasChatUnread(f.uid)?'inline-block':'none'}"></span><button class="friend-btn" onclick="openChatWith('${f.uid}','${escHtml(f.displayName)}','${escHtml(f.photoURL||'')}')">Open Chat</button></div>`;
    list.appendChild(card);
  });
  updateMessageNotifUI();
  initMessageNotificationListeners();
}
...
function openChatWith(friendUid, friendName, friendPhoto) {
  ...
  _setChatUnread(friendUid, false);
  ...
  _chatUnsub = _fdb()
    ...
    .onSnapshot(snap => {
      ...
      snap.docs.forEach(doc => {
        const m = doc.data();
        if (m && m.time) _msgLastSeenTime[friendUid] = Math.max(_msgLastSeenTime[friendUid] || 0, Number(m.time || 0));
...
function sendMsg() {
  ...
  const msg = {
    ...
    text: text,
    time: Date.now(),
    senderName: window.TF_PROFILE?.displayName || window.TF_USER?.displayName || 'Someone'
  };
...
// ════════════════════════════════════════════════════════════════════════════
let _leaderboardFriendsLoading = false;
function renderRightPanel() {
  const lb = document.getElementById('rp-lb'); if(!lb) return; lb.innerHTML = '';
  if (_uid() && !_friendsCache.length && !_leaderboardFriendsLoading) {
    _leaderboardFriendsLoading = true;
    _loadFriends().then(friends => {
      _friendsCache = friends || [];
      _leaderboardFriendsLoading = false;
      renderRightPanel();
    }).catch(() => { _leaderboardFriendsLoading = false; });
  }
  const sc = getScore(), s = getStreak();
  const me = { name:'You', avatar: window.TF_USER?.photoURL||'👤', streak:s.count, score:sc.total||0 };
  const all = [me, ...(_friendsCache.map(f=>({ name:f.displayName || 'Friend', avatar:f.photoURL||'👤', streak:Number(f.streak||0), score:Number(f.score||0) })))].sort((a,b)=>b.score-a.score);
  all.forEach((f,i) => {
    const rank = i+1;
  ...
  const act = document.getElementById('rp-activity'); if(!act) return; act.innerHTML = '';
  const activities = [
