...
// Characters
const CHARACTERS=[
  {id:'neko',emoji:'🐱',name:'Neko',rarity:'common',pts:0,bonus:'+0%',bonusVal:0},
...
function initTheme(){
  ...
  // Some builds of the UI no longer include a #theme-btn button.
  // The old code tried to update it unconditionally, which threw a
  // TypeError during init() and stopped the rest of startup from running.
  // That is why #time-col and #grid-area stayed empty and categories/blocks
  // never rendered. Keep theme setup, but only touch the button if it exists.
...
function showToast(msg,dur=2800){const t=document.getElementById('toast'); if(!t){console.log(msg); return;} t.textContent=msg;t.classList.add('show');clearTimeout(t._t);t._t=setTimeout(()=>t.classList.remove('show'),dur)}
...
function updateNotifBadge(){
  ...
  if(!cnt) return;
  // In-app notifications use a dot on the bell instead of a number.
  cnt.textContent='';
  cnt.classList.add('unread-dot-badge');
...
function renderNotifList(){
  const list=document.getElementById('notif-list');const ns=getNotifs();if(!list)return;list.innerHTML='';
...
const BLOCK_POINT_LIMIT_PER_DAY = 10;
const BLOCK_POINT_BASE = 50;
function getBlockAwardState(){
  return JSON.parse(localStorage.getItem(_pfx('tf_block_awards_v1'))||'{}');
}
function saveBlockAwardState(data){
  localStorage.setItem(_pfx('tf_block_awards_v1'), JSON.stringify(data||{}));
}
function awardBlockCreationPoints(dateKey,x,y){
  const ds = dateKey || currentDate || fmtDate(new Date());
  const data = getBlockAwardState();
  const day = data[ds] || {count:0};
  if((day.count||0) >= BLOCK_POINT_LIMIT_PER_DAY){
    data[ds] = day;
    saveBlockAwardState(data);
    updateStreakUI();
    return 0;
  }
  ...
  saveBlockAwardState(data);
  checkDayStreak();
  return awardPoints(BLOCK_POINT_BASE, x, y);
}
...
function showCharUnlock(ch){
  ...
  showModal({
    ...
    body:`<div class="char-unlock-card"><span class="char-unlock-emoji">${ch.emoji}</span><div style="font-size:16px;font-weight:700;margin-top:8px">${ch.name}</div><div class="char-rarity r-${ch.rarity}" style="font-size:11px;margin-top:3px">${ch.rarity}</div><div style="font-size:11px;color:var(--muted);margin-top:4px">${ch.bonus} passive bonus</div></div>`,
    btn:'Activate in Profile',
    onConfirm:()=>{
      const cc=getCharState();
      if(!cc.unlocked.includes(ch.id)){cc.unlocked.push(ch.id);ss('CHARS',cc);}
      const sc=getScore();
      sc.activeChar=ch.id;
      ss('SCORE',sc);
      updateStreakUI();
      if(document.getElementById('profile-inner')) renderProfile();
      showToast(`${ch.emoji} ${ch.name} activated!`);
      addNotif('✨',`${ch.name} is now your active character.`);
    }
  });
...
function checkDayStreak(){
  ...
  if(nm>om){
    const milestonePts=(nm-om)*1000;
    awardPoints(milestonePts,null,null,{ignoreBonus:true});
    setTimeout(()=>{beep('milestone');showMilestonePopup(s.count,milestonePts);addNotif('🏆',`${s.count}-day streak! +${milestonePts.toLocaleString()} pts`);},700);
  }
  updateStreakUI();
}
function updateStreakUI(){
  ...
  const tn=10-(s.count%10);document.getElementById('sb-streak-next').textContent=tn===10&&s.count>0?`🎯 Milestone: ${s.count+10}d`:`${tn}d to +1000 pts ⭐`;
