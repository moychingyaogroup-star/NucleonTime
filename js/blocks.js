...
function createBlock(x,y,text,color,catId,w,h,id,done,media,emoji,fontSize){
  ...
  if(!id && !_loadingBlocks && typeof awardBlockCreationPoints === 'function') {
    requestAnimationFrame(()=>{
      const rect = block.getBoundingClientRect();
      awardBlockCreationPoints(currentDate, rect.left + Math.min(32, rect.width/2), rect.top + 18);
    });
  }
...
function toggleBlockDone(block,chk){
  ...
  if(!isDone){beep('complete');addNotif('✅','Block completed.');}
  saveWS();
}
...
function applyBlockCat(block,catId){
  ...
  // Keep the block's original dragged-out colour. Categories should label a block,
  // not recolour it automatically.
  let newColor = { hex: block.dataset.hex || BLOCK_COLORS[0].hex, fg: block.dataset.fg || BLOCK_COLORS[0].fg, name: block.dataset.name || '' };
...
function toggleCatPanel(){
  const panel=document.getElementById('cat-side-panel');
  if(!panel) return;
  panel.classList.toggle('open');
  updateCatPanelToggle();
}
function updateCatPanelToggle(){
  const panel=document.getElementById('cat-side-panel');
  const btn=document.getElementById('cat-panel-toggle');
  if(!panel||!btn) return;
  const open=panel.classList.contains('open');
  btn.textContent=open?'›':'‹';
  btn.title=open?'Minimize categories':'Expand categories';
...
function openBlockDD(block,btnEl){
  ...
  const stGrid=document.createElement('div');stGrid.className='dd-emoji-grid';
  BLOCK_STICKERS.forEach(em=>{
    const b=document.createElement('button');b.textContent=em;
    b.className='dd-emoji-btn';
...
function buildTimeGrid(){
  ...
  // Rebuild cleanly so the time labels/grid cannot silently disappear after
  // a resize, hot reload, or repeated initialization.
  col.innerHTML='';
  grid.querySelectorAll('.grid-row').forEach(r=>r.remove());

  let nl=document.getElementById('now-line');
  if(!nl){
  ...
  const totalH=(ROW_H*24)+'px';
  col.style.height=totalH;
  col.style.minHeight=totalH;
  grid.style.height=totalH;
  grid.style.minHeight=totalH;
  ...
  for(let h=0;h<24;h++){
    ...
    row.className='grid-row'+(h===ch?' cur-hr-row':'');
    row.style.top=(h*ROW_H)+'px';
    row.style.height=ROW_H+'px';
    grid.insertBefore(row,nl);
  }
}
...
function showScheduleDatePicker(){
  const picker=document.getElementById('schedule-date-picker');
  if(!picker) return;
  picker.value=currentDate || fmtDate(new Date());
  if(typeof picker.showPicker === 'function') picker.showPicker();
  else picker.click();
}
function pickScheduleDate(ds){ if(ds) setCurrentDate(ds); }
function setCurrentDate(ds){
  currentDate=ds;
  loadWS(ds);
  if(typeof loadScheduleStickers==='function') loadScheduleStickers(ds);
  const label=document.getElementById('dnav-label');
  if(label) label.textContent=dateDisplayStr(ds);
  const picker=document.getElementById('schedule-date-picker');
