...
_auth.onAuthStateChanged(async function(user) {
  ...
  if (user) {
    ...
    if (typeof initRealNotifications === 'function') initRealNotifications();
    if (typeof syncImportantDatesForNotifications === 'function' && typeof getDates === 'function') syncImportantDatesForNotifications(getDates());
    // Friend request listener is started lazily when Friends or Messages is opened.
  } else {
