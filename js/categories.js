...
function saveDates(ds){
  ss('DATES',ds);
  document.getElementById('nb-dates').textContent=ds.filter(d=>d.pinned).length||'';
  document.getElementById('nb-dates').style.display=ds.filter(d=>d.pinned).length?'':'none';
  if (typeof syncImportantDatesForNotifications === 'function') syncImportantDatesForNotifications(ds);
  if (typeof checkImportantDateBrowserNotifications === 'function') checkImportantDateBrowserNotifications();
}
